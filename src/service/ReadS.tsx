...
const ReadS: React.FC = () => {
  ...
  const service = useSelector((state: { service: { data: tableService[] } }) =>
    state.service.data.find((item) => item.id === id)
  );

  useEffect(() => {
    dispatch(fetchPageS() as any); // Sử dụng action creator fetchPageE
  }, [dispatch]);

  const columns = [
    {
      title: "Số thứ tự",
      dataIndex: "mota", // Replace with the actual property name
      key: "mota",
    },
    {
      title: "Trạng thái",
      dataIndex: "tthd", // Replace with the actual property name
      key: "tthd",
    },
  ];

  const data = service ? [{ mota: service.mota, tthd: service.tthd }] : [];
      ...
      <Layout>
        ...
        <Layout>
          ...
          <Content>
            ...
            <Row>
              <Col span={1}></Col>
              <Col className="col_ReadE" span={6}>
            ...
            <Row>
                ...
                <Link to="/service">
                  <Button className="equipment-button">
                    <Back />
                    <Space  style={{ display: "block", width: 60 }}>
                      Quay lại
                    </Space>
                  </Button>
                  </Link>
